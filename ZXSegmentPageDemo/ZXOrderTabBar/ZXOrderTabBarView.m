...
@implementation ZXOrderTabBarView
...
- (void)AddTarBarBtn:(NSString *)name {
    ...
    [btn.titleLabel setFont:[UIFont systemFontOfSize:[ZXUIViewTool pxTopt:30.0]]];
...
- (void)layoutSubviews {
    ...
    for(int i=0;i<btnCount;i++){
        if ([self.subviews[i] isKindOfClass:[UIButton class]]) {
            ...
            view.frame = CGRectMake(0, btnH - [ZXUIViewTool pxTopt:bottomViewW], btnW,[ZXUIViewTool pxTopt:bottomViewW]);
        }
    }
}
...
- (void)TabBtnClick:(UIButton *)sender {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        self.bottomView.frame = CGRectMake(sender.tag*self.btnW, self.btnH - [ZXUIViewTool pxTopt:bottomViewW], self.btnW, [ZXUIViewTool pxTopt:bottomViewW]);
    }];
